...
spatialCode = nimbleCode({
  ...
  for (i in 1:I) 
  {
    for (j in 1:J)
    {
      ...
      log(theta[i,j])  <-  beta0 + (beta1+delta[i])*t[j] + s[i] + v[i]
    }
  }
...
inits = list( beta0=-8 , beta1=0 , taus=1 , tauv = 1 , taud = 1 ,
              delta=rep(0,I) , s=rep(0,I) , v=rep(0,I) )
...
spatial.model.Conf$addMonitors( c( "beta0" , "beta1" , "taud" , "taus" , 
                                   "delta" , "tauv" , "theta" ) )
...
taud.samp = as.vector(samples.spatial[,"taud"])
delta.1.samp = as.vector(samples.spatial[,"delta[1]"])
...
ts.plot( delta.1.samp , main="Trace Plot for delta[1]")

# windows( 6 , 6 )
plot( taus.samp , tauv.samp , log="xy" )

head( samples.spatial )
theta.hat    = apply( samples.spatial[,94:1941] , 2 , mean )
se.theta.hat = apply( samples.spatial[,94:1941] , 2 , sd )
deltahat = apply( samples.spatial[,3:90] , 2 , mean )
beta0hat = mean( samples.spatial[,1] )
beta1hat = mean( samples.spatial[,2] )
beta1hat + deltahat
...
theta.hat1 = matrix( theta.hat , nrow=I , ncol=J )   ## Reshaping theta.hat
resid = matrix( 0 , nrow=I , ncol=J )
for ( i in 1:I )  for ( j in 1:J )  resid[i,j] = LC.matrix[i,j] - theta.hat1[i,j]

# windows( 9 , 7 )
plot( theta.hat1[18,] , LC.matrix[18,] )
abline( a = 0 , b = 1 )

resid1 = as.vector( resid )
plot( resid1 )
...
for( county in 49:72)
{
  plot( 1:J , resid[county,] , ylim=c(-0.001,0.001) , 
